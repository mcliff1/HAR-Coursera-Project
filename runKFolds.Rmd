...
tic("rf model1")
rf_fit1 <- train(classe ~ ., data=training_validation[-flds[[1]], ], method="parRF")
rf_pred1 <- predict(rf_fit1, newdata=training_validation[ flds[[1]],])
toc()


tic("rf model2")
rf_fit2 <- train(classe ~ ., data=training_validation[-flds[[2]], ], method="parRF")
rf_pred2 <- predict(rf_fit2, newdata=training_validation[ flds[[2]],])
toc()


tic("rf model3")
rf_fit3 <- train(classe ~ ., data=training_validation[-flds[[3]], ], method="parRF")
rf_pred3 <- predict(rf_fit3, newdata=training_validation[ flds[[3]],])
toc()
```
...
#lda_fit$finalModel
#lda_pred
confusionMatrix(rf_pred1, training_validation[ flds[[1]],]$classe)
confusionMatrix(rf_pred2, training_validation[ flds[[2]],]$classe)
confusionMatrix(rf_pred3, training_validation[ flds[[3]],]$classe)
...
# make a dataframe using the full training_validation set
combined_data <- data.frame(
    rf1_pred = predict(rf_fit1, newdata_training_validation), 
    rf2_pred = predict(rf_fit2, newdata_training_validation), 
    rf3_pred = predict(rf_fit3, newdata_training_validation), 
    classe=training_validation$classe)


tic("fit combined model")
combined_fit <- train(classe ~ ., data=combined_data, method="parRF")
combined_pred <- predict(combined_fit, newdata=combined_data)
toc()

confusionMatrix(combined_pred, combined_data$classe)
```
Viola